...
<body style="background-color: #b9b9bd">
    ...
    <?php
        ...
        if ($_SERVER["REQUEST_METHOD"] == "POST") {
            ...
            $error_messages = array(); // Inicializa el array de mensajes de error
            ...
            // Verifica si el correo electrónico ya existe en la base de datos para esta edición y rol
            ...
            if (empty($error_messages)) {
                $query_insertar = "INSERT INTO comite (comite_nombre, comite_clave, comite_rol, comite_correo, comite_fechaRegistro, comite_activo, comite_edicion) VALUES ('$nombre', '$contrasena', 3, '$correo', now(), 1, '$edicion')";
                if (mysqli_query($db_connection, $query_insertar)) {
                    // Inserta los datos también en la tabla de usuarios (Usuario)
                    $query_insertar_usuario = "INSERT INTO usuario (usuario_nombre, usuario_clave, usuario_rol, usuario_edicion, usuario_fecha_reg, usuario_activo) VALUES ('$correo', '$contrasena', 3, $edicion, now(), 1)";
                    ...
                    $mensaje .= "<p>Has sido registrado como revisor {$usuario_id} en nuestro sistema. A continuación, encontrarás la información necesaria para acceder a tu cuenta:</p>";
                    $mensaje .= "<p><strong>Correo:</strong> {$correo}</p>";
                    $mensaje .= "<p>Para continuar con el registro de su solicitud, es necesario que proporcione los datos que se solicitan en el siguiente enlace:</p>";
    ...
    <div style="max-width: 700px; margin: 0 auto;">
        <div style="border: 1px solid #dddddd; padding: 20px; background-color: #f5f5f5; border-radius: 20px; box-shadow: 0px 0px 10px 0px rgba(0,0,0,0.75);">
            ...
            <h1 style="text-align: center">Formulario del Revisor</h1>
            
            <p style="text-align: center">Completa todos los campos requeridos para proceder con el registro.</p>

            <h2 style="margin-left: 30px; margin-bottom: -10px;">Información del revisor</h2> 
            ...
            <form id="estiloEduardo.css" class="form-control-1" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" style="display: inline-block; text-align: left"> 
                ...
                <div style="text-align: center;">
                    <input type="submit" value="Alta revisor" class="w3-button w3-btn w3-ripple w3-theme-slightly-dark-red w3-round-large" style="margin-right: 30px;">
